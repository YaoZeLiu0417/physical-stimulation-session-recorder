...
- 被试可用带签名链接 ?sid=...&exp=...&sig=... 直接进入（免口令）
- 管理员/调试仍可用 APP_PASSWORD_SHA256 口令进入
- 录制：浏览器→服务器落盘(FLV)→(可选 ffmpeg 转码 MP4)
- 上传：百度网盘 precreate/superfile2/create 分片 + 指数退避重试
- 上传成功后默认删除服务器本地副本（视频+JSON，可取消）
"""
...
# =========================
st.set_page_config(
    page_title="实验干预录制 & 上传（NLP 采集 / 链接锁定 / 重试）",
    page_icon="🎥",
    layout="centered",
)
...
def _safe_secret(key: str, default: str = "") -> str:
    ...
    try:
        return st.secrets[key]  # 可能抛异常（本地无 secrets.toml）
    except Exception:
...
# =========================
LINK_SIGNING_KEY = _safe_secret("LINK_SIGNING_KEY", "")  # 在环境变量或 Secrets 配置
...
def verify_link_params() -> tuple[Optional[str], str]:
    """校验 URL 中的 sid/exp/sig；通过则返回 (sid, "")，否则 (None, why)。"""
    q = _get_query_params()
    sid = q.get("sid", "")
    exp_raw = q.get("exp", "")
    sig = q.get("sig", "")
...
def require_app_password():
    ...
    if sid:
        ...
        st.session_state["subject_id"] = sid  # 后续输入框会锁定
        return

    # ② 否则才需口令（研究者/调试）
    expected_hash = _safe_secret("APP_PASSWORD_SHA256", "")
    if not expected_hash:
        return  # 未配置口令则放行（仅限内部/本地）
...
if not CONF_PATH.exists():
    st.error("缺少 config.toml，请在项目根目录创建并填写 [baidu] 配置。")
    st.stop()

CFG = toml.loads(CONF_PATH.read_text(encoding="utf-8")).get("baidu", {})
AK = CFG.get("app_key", "")
...
if not AK or not SK:
    st.error("请在 config.toml 中填写 baidu.app_key / baidu.secret_key。")
    st.stop()
...
def ensure_token() -> str:
    """用 refresh_token 刷新 access_token，并回写 config.toml。"""
    conf_all = toml.loads(CONF_PATH.read_text(encoding="utf-8"))
    conf = conf_all.get("baidu", {})
    refresh_token = conf.get("refresh_token", "")
    if not refresh_token:
        raise RuntimeError("缺少 refresh_token：请先运行 bd_init.py 完成一次授权。")
    ...
    if "access_token" not in j:
        raise RuntimeError(f"刷新 token 失败：{j}")

    conf["access_token"] = j.get("access_token", "")
    if j.get("refresh_token"):
        conf["refresh_token"] = j["refresh_token"]
    conf_all["baidu"] = conf
    CONF_PATH.write_text(toml.dumps(conf_all), encoding="utf-8")
...
- **网盘路径**：`{save}/<被试>/<YYYYMMDD>/视频与同名_state.json`。 其中 `{save}` = `config.toml` 的 `save_dir`。
- **Token 刷新**：每次上传前自动用 `refresh_token` 刷新 `access_token`；`refresh_token` 本身也会更新并回写 `config.toml`。
